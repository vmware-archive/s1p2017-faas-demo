...
pushd functions/vote-counter ; riff build -v 0.0.7 ; popd
pushd functions/redis-writer ; riff build -v 0.0.6; popd
pushd functions/vote-stream-processor ; ./mvnw -DskipTests clean package ; riff build -v 0.0.6 ; popd
pushd ui ; docker build -t projectriff/riff-demo-ui:0.0.9 . ; popd
```
